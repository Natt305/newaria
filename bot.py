...
_CHAR_IMAGE_EXTRACTION_QUESTION = (
    ...
    "Analyze this character reference image and return a precise description "
    "suitable for use as an image generation prompt. "
    "Output ONE single continuous paragraph — no headers, no bullet points, no intro.\n\n"

    "=== CRITICAL COLOUR RULES — READ FIRST ===\n\n"

    "HAIR COLOUR — near-white/silver trap:\n"
    "Anime characters with very pale, low-saturation hair are almost never truly "
    "silver or white — they have a subtle colour tint (mint-green, lavender, pale blue, "
    "pale rose, cream, etc.). Silver hair is only correct if the hair is a true "
    "neutral grey-silver with absolutely no colour tint whatsoever.\n"
    "STEP 1: Does the hair appear near-white or silver? If YES →\n"
    "STEP 2: Look for any color tint in the highlights, shadows, or overall tone. "
    "Is there any green? Any blue? Any purple? Any pink? Even very subtle?\n"
    "STEP 3: Describe the ACTUAL tint. Examples:\n"
    "  - Slight cool blue-green tint → 'pale seafoam mint-green, very low saturation'\n"
    "  - Slight purple tint → 'pale lavender, very low saturation'\n"
    "  - Slight pink tint → 'pale rose, very low saturation'\n"
    "  - Truly neutral, no tint → 'silver-white, neutral'\n"
    "NEVER write just 'silver' or 'white' if any colour tint is detectable.\n\n"

    "EYE COLOUR — vivid vs muted trap:\n"
    "Anime eyes that look golden or amber are often actually MUTED warm olive-brown "
    "at very low saturation — NOT vivid golden-amber. Writing 'golden' or 'amber' "
    "causes AI generators to render bright vivid eyes, which is wrong for muted eyes.\n"
    "STEP 1: Are the eyes yellow-orange-brown in the hue family? If YES →\n"
    "STEP 2: Judge saturation — are they brilliantly vivid like a jewel, or soft/muted/earthy?\n"
    "STEP 3: If muted/earthy, write: 'very low-saturation muted warm olive-brown with "
    "yellowish-green undertone, soft, medium brightness'\n"
    "NEVER write 'golden', 'amber', or 'honey' alone — always state the saturation level.\n\n"

    "CAPE vs COAT — sleeve test:\n"
    "Look at where the character's upper arm meets the shoulder piece. "
    "If the upper arm is BARE/EXPOSED while the shoulder has a piece of fabric → it is a CAPE. "
    "Capes drape from the shoulder but leave the arm free. "
    "Only write 'coat' if you can see fabric covering the entire arm down to the wrist.\n\n"

    "=== NOW DESCRIBE THE CHARACTER ===\n\n"

    "Cover in this order:\n"
    "1. HAIR: Apply the near-white/silver trap check above. State exact shade, length "
    "(very long past waist / shoulder / short bob), cut, bang style, "
    "and 'NO ahoge' or 'ahoge present'.\n\n"

    "2. EYES: Apply the vivid vs muted trap check above. Three axes required: "
    "hue family + saturation + brightness.\n\n"

    "3. EYELASHES: hue (cool-haired characters often have cool greenish-grey lashes, "
    "not warm brown) + explicitly state 'NOT black or dark brown' if applicable + weight.\n\n"

    "4. SKIN: exact shade and undertone (warm/cool/neutral).\n\n"

    "5. OUTFIT from outermost to innermost, each piece as its own entry "
    "(colour + type + material + fit + details):\n"
    "  - Apply the cape vs coat sleeve test for the outer layer.\n"
    "  - Are there separate structured shoulder pads/epaulettes? List as separate entry.\n"
    "  - Chest ruffle/jabot: state size (palm-sized covering sternum only? or larger?). "
    "State EXACT brooch/gemstone colour (green / red / blue — be precise).\n"
    "  - Lower body: are coloured shorts visible? Is a short skirt layered over them?\n"
    "  - Stockings: EXACT shade (dark grey ≠ black).\n"
    "  - Shoes: type, colour, heel, straps.\n\n"

    "6. ART STYLE: rendering type, line art, shading, palette, distinctive markers.\n\n"

    "Join traits with '; ' and outfit pieces with ' + '. "
    "One paragraph only. No vague colour names."
)
def _cf_ready():

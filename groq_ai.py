...
async def enhance_image_prompt(
    ...
    if n_subjects_hint >= 2:
        ...
        if has_images:
            multi_char_note += (
                "\n"
                f"RULE — POSITION LABELS (mandatory) + NO APPEARANCE TEXT for photo characters:\n"
                "The reference photos are the SOLE source of character appearance for characters "
                "that have reference photos. Writing appearance in text OVERRIDES the photos and causes errors.\n"
                "For each character, write ONLY their spatial position label and name: "
                "'(left) [Name]' and '(right) [Name]'. Nothing else about their appearance.\n"
    ...
    if n_subjects_hint >= 2 and has_images:
        _checklist = (
            ...
            "  ✓ POSITION LABELS — '(left) [Name]' and '(right) [Name]' are present; "
            "no other appearance text for photo-based characters (photos carry appearance)\n"
            ...
            "  ✓ NO APPEARANCE TEXT for photo characters — hair/eyes/outfit descriptions "
            "for photo-based characters are ABSENT (they override the reference photos)\n"
            "If any item is missing or if you wrote appearance text for a photo-based character, fix it.\n"
        )
    else:
        _checklist = (
    ...
    if has_images:
        if n_subjects_hint >= 2:
            ...
            user_content = (
                ...
                "CRITICAL — REFERENCE PHOTOS CARRY APPEARANCE:\n"
                "The image model reads character appearance (hair, eyes, skin, outfit) DIRECTLY from the "
                "attached reference photos via its own mechanism. Any appearance text you write for a "
                "photo-based character OVERRIDES the photos, causing wrong colors and wrong outfits in the "
                "output. You MUST NOT describe appearance for photo-based characters.\n\n"
                "Build your paragraph with:\n\n"
                "  (1) POSITION LABELS — write '(left) [Name]' and '(right) [Name]' (matching the spatial "
                "prefix in the text if present). That is the ONLY text about photo-based characters' identity. "
                "Do NOT add hair color, eye color, skin tone, or outfit for photo-based characters.\n\n"
                "      EXCEPTION: If a character has a [SUBJECT REFERENCE] block in the system prompt "
                "(they have NO reference photos), reproduce their FULL appearance from that block — "
                "hair, eyes, skin tone, and every garment listed. This text IS the image model's only "
                "source for that character.\n\n"
                "  (2) SCENE — the majority of your output. Richly expand from the text below:\n"
                "      setting, background, environment, lighting, atmosphere, mood, what the characters\n"
                "      are doing, how they interact, poses, expressions, emotional tone.\n"
                "      Props and instruments mentioned in the text MUST appear.\n"
                "      DISCARD any appearance words from the raw text — they were written without photos.\n\n"

...
async def enhance_image_prompt(
    ...
    if n_subjects_hint >= 2:
        ...
        if has_images:
            multi_char_note += (
                "\n"
                "RULE — READ EACH CHARACTER FROM THEIR REFERENCE PHOTO:\n"
                "Reference photos are attached. Treat each photo as a single character's appearance source.\n"
                "For EACH photo-based character, read their appearance directly from the photo — "
                "exactly as you would in a single-character request — and write:\n"
                "  1. Spatial position label + name: '(left) [Name]' or '(right) [Name]'\n"
                "  2. Hair colour — read from the photo\n"
                "  3. Outfit — read from the photo, described piece by piece\n"
                "Each character MUST have their own distinct appearance block derived from their own photo. "
                "NEVER merge or share outfit descriptions across characters. "
                "NEVER write 'both wearing X' — each character's photo shows their own outfit.\n"
                "DO NOT write eye colour or skin tone (those are harder to differentiate in multi-char and cause bleed).\n"
    ...
    if n_subjects_hint >= 2 and has_images:
        _checklist = (
            ...
            "  ✓ PER-CHARACTER PHOTO READING — each photo-based character has their own appearance block:\n"
            "      • position label + name\n"
            "      • hair colour read from their reference photo\n"
            "      • outfit read from their reference photo, described piece by piece\n"
            "  ✓ DISTINCT OUTFITS — each character's outfit description is different and comes from their own photo. "
            "NEVER write 'both wearing X' or any shared outfit phrase.\n"
            "  ✓ NO MERGED APPEARANCE — eye colour and skin tone are ABSENT (cause bleed in multi-char)\n"
            ...
            "If any item is wrong, fix it before finalizing.\n"
        )
    else:
        _checklist = (
    ...
    if has_images:
        if n_subjects_hint >= 2:
            ...
            # Strategy: scene-first with position labels only for photo-characters.
            # The reference photos carry ALL appearance via ReferenceLatent — writing
            # appearance text for photo-characters overrides the photos and causes errors
            # (LLM mis-reads of compressed thumbnails appear directly in the output image).
            # Only characters with [SUBJECT REFERENCE] blocks (no photos) get appearance text.
            user_content = (
                "Image request (reference photos attached above).\n\n"
                "Your output is a single continuous paragraph (no headers, no bullet points).\n\n"
                "THIS IS A MULTI-CHARACTER SCENE WITH REFERENCE PHOTOS.\n\n"
                "HOW THE IMAGE MODEL WORKS: It reads character appearance (eyes, skin, outfit, hairstyle) "
                "DIRECTLY from the attached reference photos. Any appearance text you write for those traits "
                "OVERRIDES the photos — causing wrong colours and wrong outfits in the final image.\n\n"
                "EXCEPTION — HAIR COLOUR ONLY: Hair colour is the one trait the image model can default "
                "wrongly if the text gives no anchor (it falls back to dark hair). For each photo-based "
                "character, if their [SUBJECT APPEARANCE SUPPLEMENT] block exists in the system prompt, "
                "copy ONLY the hair colour phrase from that block verbatim. "
                "Do NOT infer hair colour from the photos yourself. Do NOT add any other appearance.\n\n"
                "Build your paragraph in this order:\n\n"
                "  (1) POSITION + HAIR ANCHOR — for each photo-based character:\n"
                "      '(left) [Name], [hair colour from supplement]' and '(right) [Name], [hair colour]'.\n"
                "      If no supplement exists, write position label and name only.\n"
                "      Characters with [SUBJECT REFERENCE] blocks (no photos): reproduce their FULL\n"
                "      appearance from that block — hair, eyes, skin, every garment listed.\n\n"
                "  (2) SCENE — the majority of your output. Richly expand from the text:\n"
                "      setting, environment, lighting, atmosphere, mood, poses, expressions, interaction.\n"
                "      Props and instruments mentioned in the text MUST appear.\n"
                "      DISCARD appearance words from the raw text — written without photos, unreliable.\n"
                "      ABSOLUTE RULE: DO NOT add any person, figure, silhouette, crowd, audience member,\n"
                "      bystander, or background character that is not named in the character list above.\n"
                "      The image must contain EXACTLY the named characters and nobody else.\n"
                "      Adding any extra human figure makes the output WRONG.\n\n"
                "  (3) ART STYLE — always end with: 'clean 2D anime illustration, flat cel-shaded,\n"
...
async def generate_suggestions(
    ...
    Groq's chat-call shape and json_object toggle. The shared helper owns
    the prompt construction, JSON-parse → salvage pipeline, and `[Groq]`
    log lines.
    ...
    """
    use_json_mode = _json_mode_enabled()
    # Groq's json_object mode forces an OBJECT root, so ask the prompt for
    # `{"items":[...]}` instead of a bare array. The shared parser accepts
    # both shapes so the off-path keeps emitting bare arrays.
    response_format = {"type": "json_object"} if use_json_mode else None

...
async def enhance_image_prompt(
    ...
    if has_images:
        image_note = (
            ...
            "Identify the rendering style from the reference photos. "
            "Distinguish between: 2D cell-shaded illustration, soft painterly 2D, 3D rendered anime (smooth 3D shading, volumetric lighting, no visible 2D outlines), "
            "Live2D / VTuber model style, sketch, etc. "
            "If the reference looks like a 3D render or game character model, do NOT write '2D art' — write the 3D style instead. "
    ...
    system = (
        ...
        "  * RENDERING TYPE: e.g. 'clean 2D cell-shaded anime illustration', 'soft painterly 2D digital illustration', "
        "'rough 2D sketch-style line art', 'polished semi-realistic 2D anime', 'flat graphic 2D anime', "
        "'smooth 3D rendered anime character (game/VTuber model style, volumetric 3D lighting, no 2D outlines)', "
        "'Live2D rigged character render'. "
        "CRITICAL: if the reference image has smooth 3D shading, depth, and no visible ink outlines, it is a 3D render — "
        "do NOT call it '2D art' or 'cell-shaded'. Write '3D rendered anime character' instead.\n"
        "  * LINE ART: '2D: clean sharp black outlines / soft anti-aliased thin lines / thick expressive ink lines'; "
        "'3D: no visible 2D outlines, smooth surface transitions between light and shadow'\n"
        ...
        "  2. GARMENT TYPE: exact garment name (e.g. 'fitted blazer', 'A-line pleated skirt', 'sailor-collar blouse', 'over-knee socks')\n"
        ...
        "outfit: structured deep crimson wool beret, flat-topped, with a small round polished gold pin badge centered on the front panel + "
        "slim-fit high-collar charcoal black matte blazer with narrow gold braid trim along both lapels and cuff edges, single column of small polished gold dome buttons down the center front, structured shoulders + "
        "fitted off-white sheer chiffon blouse with a small ruffled collar, visible peeking above the blazer neckline, delicate fabric + "
        "A-line pleated black opaque cotton mini skirt, waistband with a thin gold-thread stripe, skirt hem ending mid-thigh + "
        "sheer black nylon thigh-high stockings with a subtle matte sheen, smooth texture + "
        "black smooth patent-leather lace-up ankle boots, low square block heel, gold metal eyelet hardware on each lace hole, "
        "anime art style\n"
        "CRITICAL REMINDER — OUTFIT: Every piece in the Good output above has all 5 axes described. "

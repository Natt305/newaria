...
# ---------------------------------------------------------------------------
# Suspension / restoration regexes — shared with character_state.py.
# Defined centrally in _state_regexes.py; imported here to avoid drift.
# ---------------------------------------------------------------------------

from _state_regexes import (
    _CAPTIVE_TRANSITION_RE,
    _FREED_TRANSITION_RE,
    _PORTABLE_ITEM_RE,
)
...
async def update_state(
    ...
    1. Pre-flight: deterministic freed restoration — runs before the extractor
       ...
       promote suspended_accessories even if the text carries no generic appearance
    ...
    if has_freed:
        ...
        if not will_extract:
            ...
            _record_history(channel_id, discord_id, turn, original_snap, current)
            return current
    ...
    if not delta:
        ...
        if has_freed:
            _record_history(channel_id, discord_id, turn, original_snap, get_state(channel_id, discord_id))
    ...
    # `delta_snap_acc` tracks accessories AFTER restoration (pre-delta) so captive
    ...
    if updated.suspended_accessories and _FREED_TRANSITION_RE.search(exchange_text):
        ...
        if extra_promoted:
            print(f"[PlayerState] Belt-and-suspenders restored post-delta for {discord_id}: {extra_promoted}")
        else:
    ...
    _record_history(channel_id, discord_id, turn, original_snap, updated)
    return updated

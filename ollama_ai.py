...
async def enhance_image_prompt(
    ...
    system = (
        ...
        "NOT a coat (which has sleeves and a closed front). If the reference shows an open-front "
        "shoulderpiece with no sleeves that hangs down the back/sides, call it 'open-front cape' or 'shoulder mantle'.\n"
        "  • JABOT = a decorative ruffled chest piece worn at the center of the chest/sternum, "
        "secured with a brooch or pin. NOT a 'collar' (which is at the neck only). "
        "When you see a ruffled white fabric piece at chest level with a gem or pin, call it 'chest-level jabot' or 'ruffled jabot'.\n"
        "  • EPAULETTES / SHOULDER COVERS = structured decorative pieces sitting on top of the shoulders, "
        "attached to the garment. If you see padded or decorated shoulder caps with gold trim, call them 'structured shoulder epaulettes with gold decoration' "
        "and list them as a SEPARATE entry, do NOT merge them into the jacket or cape.\n"
        "  • SHORTS = visible leg garment ending above mid-thigh. "
        "If red fabric with gold buttons is visible at the lower body, it is likely shorts, NOT a vest.\n"
        "  • BROOCH RULE: If you see a gemstone pin or brooch, identify what it is fastening. "
        "If it pins a white ruffled fabric piece at the chest/sternum, write that as TWO entries: "
        "(1) 'chest-level white gathered ruffled jabot, soft sheer fabric, delicate ruffle cascade' AND "
        "(2) include the brooch in that entry's details: 'secured at center with a large green gemstone brooch'. "
        "NEVER attribute the brooch to a red or colored garment when it is clearly fastening a white fabric piece.\n"

...
async def enhance_image_prompt(
    ...
    system = (
        ...
        "- OUTFIT (mandatory, extremely detailed, piece-by-piece): list EVERY garment piece "
        "separately — do not merge or summarize. For each piece state: exact color(s), garment type, "
        "material/texture if visible, and any distinctive details (buttons, trim, embroidery, gemstones, "
        "lace, zippers, patterns, logos, ruffles, bows, etc.). Include layering order, fit, and ALL "
        "accessories (hat, belt, bag, jewelry, gloves, wrist cuffs, socks, shoes, etc.). "
        "Format: list each piece separated by commas or ' + ', one piece per entry. "
        "Example for a gothic outfit: 'black-and-crimson gothic ensemble — white pleated jabot with "
        "green gemstone brooch at center, short black cape with gold-embroidered border, beige puff-sleeve "
        "blouse underneath, red shorts layered under long black skirt, multiple gold buttons along "
        "skirt waistband, black over-knee socks, dark crimson beret'. "
        "NEVER collapse an outfit into a single vague phrase like 'gothic uniform' or 'casual clothes' "
        "— every visible piece must be listed individually.\n"
